...
class OperationDao implements Dao<Operation> {
    ...
    public void create(Operation operation) {
        try (Session session = ConnectorManager.openSession()) {
            ...
            try {
                if (operation.getType() == TypeOperation.INCOME) {
                    account.setBalance(account.getBalance().add(amount));
                } else if (operation.getType() == TypeOperation.EXPENSE) {
    ...
    public Optional<Operation> update(Operation operation) {
        try (Session session = ConnectorManager.openSession()) {
            ...
            if (updateOperation.getType() == TypeOperation.INCOME) {
                try {
                    account.setBalance(account.getBalance().subtract(amount));
                } catch (NegativeValueException e) {
                    throw new RuntimeException(e);
                }
            } else if (updateOperation.getType() == TypeOperation.EXPENSE) {
                try {
                    account.setBalance(account.getBalance().add(amount));
                } catch (NegativeValueException e) {
                    throw new RuntimeException(e);
    ...
    public boolean transferAmounts(Account currentAccount, Account targetAccount, BigDecimal amount) {
        try (Session session = ConnectorManager.openSession()) {
            ...
            if (currentAccount.getBalance().compareTo(amount) < 0) {
                System.out.println("The balance don`t have enough money");
                transaction.rollback();
                return false;
            }
            ...
            outputOperation.setType(TypeOperation.EXPENSE);
            ...
            inputOperation.setType(TypeOperation.INCOME);
    ...
    public List<Operation> findOperationsByUserAndPeriod(User user, LocalDateTime startDate, LocalDateTime endDate) {
        try (Session session = ConnectorManager.openSession()) {
            ...
            for (Operation operation : operations) {
                ...
                List<Operation> userOperations = userOperationsMap.computeIfAbsent(findUser, k -> new ArrayList<>());
            ...
            for (Map.Entry<User, List<Operation>> entry : userOperationsMap.entrySet()) {
                ...
                System.out.println("End operations list.");
                System.out.println("-------------------------------------");
            }
            return operations;
        } catch (Exception e) {
